...
/**
 * A simple system for delayed actions. Also has option to allow faster (or slower) action running if nobody is within a given range and you can even add a list
 * of exceptions to who can be in that range.
 * 
 * @author Tulir293
 * @since 0.1
 */
public class DelayedAction {
	 ...
	 * @param delay The delay in ticks to use for teleportation by default. This may be overriden by safeDelay if certain conditions are met.
	 ...
	 * @param safeRange If there are no players in the range or all the players in the range are in the noCancel array, the player will use the safeDelay
	 *            instead for delay for the delay of the teleportation. Setting to 0 or less disables the safeRange system.
	 * @param safeDelay The delay in ticks to use instead if all the players in the safeRange are in the noCancel list. Setting to the same as delay or -1
	 *            disables the safeRange system.
	 * @param noCancel The list of players who can be near the player without disallowing use of safeDelay as delay. Can be null and will not affect the status
	 *            of the safeRange system if null or empty.
	 * @throws IllegalArgumentException if p is null.
	 */
	public DelayedAction(Player p, int delay, Runnable onSuccess, Runnable onFail, double safeRange, int safeDelay, List<UUID> noCancel) {
	 ...
	 * @throws IllegalArgumentException if p is null.
	 */
	public DelayedAction(Player p, int delay, Runnable onSuccess, Runnable onFail, double safeRange, int safeDelay) {
		this(p, delay, onSuccess, onFail, safeRange, safeDelay, null);
	}
	 ...
	 * @throws IllegalArgumentException if p is null.
	 */
	public DelayedAction(Player p, int delay, Runnable onSuccess, Runnable onFail) {
		this(p, delay, onSuccess, onFail, -1, -1, null);
	}
