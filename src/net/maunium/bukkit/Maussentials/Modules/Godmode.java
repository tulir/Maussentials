...
public class Godmode implements PlayerCommandModule, Listener {
	...
	public boolean execute(Player sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("default") || args[0].equalsIgnoreCase("def")) {
				...
				sender.sendMessage(plugin.translateStd("god." + (toggle(DEFAULT_GOD, sender) ? "on.def" : "off")));
				...
				if (p != null) {
					if (args.length == 1) {
						if (p.hasMetadata(PRIVATE_GOD)) sender.sendMessage(plugin.translateErr("god.private", p.getName()));
						else sender.sendMessage(plugin.translateStd("god.for." + (toggle_def(p) ? "off" : "on.def"), p.getName()));
					} else {
						if (args[1].equalsIgnoreCase("default")) {
							if (p.hasMetadata(PRIVATE_GOD)) sender.sendMessage(plugin.translateErr("god.private", p.getName()));
							else sender.sendMessage(plugin.translateStd("god.for." + (toggle(DEFAULT_GOD, p) ? "on.def" : "off"), p.getName()));
						} else if (args[1].equalsIgnoreCase("damage")) {
							if (p.hasMetadata(PRIVATE_GOD)) sender.sendMessage(plugin.translateErr("god.private", p.getName()));
							else sender.sendMessage(plugin.translateStd("god.for." + (toggle(DAMAGE_GOD, p) ? "on.dmg" : "off"), p.getName()));
						}
					}
				} else sender.sendMessage(plugin.translateErr("god.notfound", args[0]));
			}
		}
		return true;
	}
	...
	public void help(Player sender, Command cmd, String label, String[] args) {
		sender.sendMessage(plugin.translateErr("god.help", label));
	}
