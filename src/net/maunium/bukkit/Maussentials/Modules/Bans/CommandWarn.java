...
public class CommandWarn implements CommandExecutor {
	...
	public CommandWarn(Maussentials plugin, MauBans host) {
		this.plugin = plugin;
		this.host = host;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (!plugin.checkPerms(sender, "maussentials.bans.warn")) return true;
		if (args.length == 1) {
			...
			host.warn(u, sender instanceof Player ? ((Player) sender).getUniqueId().toString() : "CONSOLE", reason, System.currentTimeMillis() + time);
			
			if (!silent)
				plugin.getServer().broadcast(
						plugin.translatePlain("bans.broadcast.warned", p.getName(), reason, sender.getName(),
								DateUtils.getDurationBreakdown(time, DateUtils.MODE_FOR)), "maussentials.bans.see.warn");
			
			if (p.isOnline())
				p.getPlayer().kickPlayer(
						plugin.translatePlain("bans.warn.temporary", reason, sender.getName(), DateUtils.getDurationBreakdown(time, DateUtils.MODE_FOR),
								p.getUniqueId()));
			return true;
		} else {
			sender.sendMessage(plugin.translateErr("bans.help.warn", label));
			return true;
		}
	}
}
